...
export const FIosDialog = ({
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent showCloseButton={false} className="sm:px-8 max-w-full theme-wrapper bg-template-primary sm:max-w-7xl mx-auto sm:min-h-[95vh] h-[80vh] overflow-y-auto flex flex-col">
                {/* Header */}
                <DialogHeader className="p-4 border-b">
                    <DialogTitle className="text-left text-3xl flex flex-col gap-2">
                        <span>
                            {blog?.title ||
                                project?.title ||
                                service?.title ||
                    ...
                    <div className="p-4 border-t flex justify-start">
                        {project.btnLink.startsWith("#") ? (
                            <Button
                                ...
                                onClick={() => handleInternalLink(project.btnLink!)}
                            >
                                {project.btnText}
                            </Button>
                        ) : (
                            ...
                            >
                                <Button className="rounded-full">{project.btnText}</Button>
                            </a>
                        )}
                    </div>
                )}
                ...
                {blog && blog.btnLink && (
                    <div className="p-4 border-t flex justify-start">
                        {blog.btnLink.startsWith("#") ? (
                            <Button
                                className="rounded-full"
                                onClick={() => handleInternalLink(blog.btnLink!)}
                            >
                                {blog.btnText}
                            </Button>
                        ) : (
                            <a href={blog.btnLink} target="_blank" rel="noopener noreferrer">
                                <Button className="rounded-full">{blog.btnText}</Button>
                            </a>
                        )}
                    </div>
                )}
                ...
                {service && service.btnLink && (
                    <div className="p-4 border-t flex justify-start">
                        {service.btnLink.startsWith("#") ? (
                            <Button
                                className="rounded-full"
                                onClick={() => handleInternalLink(service.btnLink!)}
                            >
                                {service.btnText}
                            </Button>
                        ) : (
                            <a
                                ...
                                rel="noopener noreferrer"
                            >
                                <Button className="rounded-full">{service.btnText}</Button>
                            </a>
                        )}
                    </div>
                )}
