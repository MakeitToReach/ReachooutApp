...
router.post("/check-file", isAuthenticated, tempUpload.single("file"), async (req: any, res: any) => {
  try {
    const user = req.user;
    ...
    res.status(500).json({ message: "File check failed", error: error instanceof Error ? error.message : "Unknown error" });
  }
});

router.post("/upload", isAuthenticated, tempUpload.single("file"), async (req: any, res: any) => {
  try {
    const user = req.user;
    const file = req.file as Express.Multer.File;
